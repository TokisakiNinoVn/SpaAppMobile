...
class _AddBankScreenState extends State<AddBankScreen> {
  ...
  Future<void> _addBank() async {
    ...
    try {
      ...
      final response = await _informationService.addBank(data);

      if (response['status'] == 'success') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Thêm ngân hàng thành công'),
              backgroundColor: Colors.green,
              duration: Duration(seconds: 2),
            ),
          );
          // Trả về true để báo hiệu thêm thành công và quay lại màn hình trước
          Navigator.pop(context, true);
        }
      } else {
        throw Exception(response['message'] ?? 'Thêm ngân hàng thất bại');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
