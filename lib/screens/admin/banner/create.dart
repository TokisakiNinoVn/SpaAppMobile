...
class _CreateBannerScreenState extends State<CreateBannerScreen>
  ...
  bool _hasUploadedImage = false; // Flag để kiểm tra đã upload ảnh mới chưa
    ...
    try {
      ...
      if (imageData != null) {
        ...
        if (_uploadedFileId != null) {
          await fileService.deleteFileService(_uploadedFileId!);
        }
  ...
  Future<bool> _onWillPop() async {
    // Nếu đã upload ảnh mới và chưa tạo banner thì hiện xác nhận
    if (_hasUploadedImage && !_isLoading) {
      final shouldExit = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: const Text('Xác nhận thoát'),
          content: const Text('Bạn đã tải lên ảnh mới nhưng chưa tạo banner. Bạn có muốn thoát không?'),
          actions: [
            TextButton(
