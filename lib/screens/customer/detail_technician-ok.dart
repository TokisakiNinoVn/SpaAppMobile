...
class DetailsTechnicianScreen extends StatefulWidget {
  ...
  final Id;
  const DetailsTechnicianScreen({super.key, required this.tech, required this.Id});
...
class _DetailsTechnicianScreenState extends State<DetailsTechnicianScreen> {
  ...
  Future<void> loadListTechnician(String Id) async {
    try {
      final response = await _technicianService.getDetailsTechnicianService(widget.tech["_id"]);

    } catch (e) {
      print('Error loading list technician: $e');
    }
  }
  Future<void> _toggleFavorite(String techId) async {
    try {
      final payload = {"technicianId": techId};
  ...
  void _confirmBooking() {
    if (_selectedService != null && _selectedTimeIndex != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Đã đặt dịch vụ ${_selectedService!["name"]} thành công',
            style: const TextStyle(color: Colors.white),
          ),
      ...
      Future.delayed(const Duration(milliseconds: 1500), () {
        context.go(
          '/home-customer/list-technician/detail-technician/create-order-technician',
          extra: {
            'technician': widget.tech,
  ...
  String _getAvatarUrl() {
    if (widget.tech["avatar"] != null && widget.tech["avatar"]["url"] != null) {
      return widget.tech["avatar"]["url"];
    }
    if (widget.tech["images"] != null && widget.tech["images"].isNotEmpty) {
      return widget.tech["images"][0]["url"];
    }
  ...
  List<String> _getImageUrls() {
    ...
    // Thêm avatar nếu có
    if (widget.tech["avatar"] != null && widget.tech["avatar"]["url"] != null) {
      urls.add(widget.tech["avatar"]["url"]);
    }

    // Thêm các ảnh khác
    if (widget.tech["images"] != null) {
      for (var image in widget.tech["images"]) {
        if (image["url"] != null) {
          urls.add(image["url"]);
        }
      }
  ...
  String _getGenderText() {
    final gender = widget.tech["gender"] ?? "";
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            ...
            right: 16,
            child: InkWell(
              onTap: () => _toggleFavorite(widget.tech["_id"]),
              child: Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.tech["fullName"] ?? "Không có tên",
                      style: TextStyle(
                        fontSize: 28,
                    ...
                    Text(
                      "Kinh nghiệm: ${widget.tech["experience"] ?? "Không có"}",
                      style: TextStyle(
                        fontSize: 14,
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              if (widget.tech["yearOfBirth"] != null)
                _buildInfoChip(
                  icon: Icons.cake,
                  text: "${widget.tech["yearOfBirth"]}",
                ),
              ...
              if (widget.tech["province"] != null)
                _buildInfoChip(
                  icon: Icons.location_city,
                  text: widget.tech["province"],
                ),

              // if (widget.tech["districts"] != null && widget.tech["districts"].isNotEmpty)
              //   _buildInfoChip(
              //     icon: Icons.location_on,
              //     text: "${widget.tech["districts"].length} quận",
              //   ),
            ],
          ),
  ...
  Widget _buildDescription() {
    final bio = widget.tech["bio"] ?? "Không có thông tin giới thiệu";
  ...
  Widget _buildServicesSection() {
    final services = widget.tech["service"] ?? [];
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_selectedService!["description"] != null && (_selectedService!["description"] as String).isNotEmpty)
              Padding(
