...
class _HomeTechnicianTabState extends State<HomeTechnicianTab> {
  ...
  Future<void> _getCurrentLocation(BuildContext context) async {
    ...
    try {
      ...
      final Position location = await Geolocator.getCurrentPosition(
        desiredAccuracy:
            LocationAccuracy
                .high, // Quan trọng: dùng high để trigger Location Accuracy
  ...
  Future<void> _updateLocation() async {
    ...
    try {
      final data = {"lat": currentLat, "lng": currentLng};
      final response = await technicianService.updateLocationTechnicianService(
        data,
      );
  ...
  Future<void> _checkApprovalStatus() async {
    ...
    if (now - lastCheck < 10 * 60 * 1000) {
      ...
      SnackBarHelper.showError(
        context,
        "Bạn cần chờ hết thời gian đếm ngược để kiểm tra lại.",
      );
      return;
    }
    ...
    try {
      final response =
          await userService.getIsAcceptHaveApprovalRequestService();
      if (response['success'] == true) {
        ...
        if (isAccept) {
          SnackBarHelper.showSuccess(
            context,
            "Tài khoản đã được phê duyệt, vui lòng đăng nhập lại",
          );
  ...
  Future<void> _loadUserDetail() async {
    ...
    idOrderWorking =
        await SharedPrefs.getValue(PrefType.string, "idOrderWorking") ?? "";
    ...
    //   final orderDetailRaw = await SharedPrefs.getValue(
    //     PrefType.string,
    //     "orderDetail",
    //   );
    ...
    //     try {
    //       orderDetail =
    //           jsonDecode(orderDetailRaw.toString()) as Map<String, dynamic>;
    ...
    try {
      ...
      if (response['success'] == true) {
        setState(() {
          ...
          // appLog("Detail technician: $technicianData");
          isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> toggleUserStatus() async {
    ...
    if (response['success'] == true) {
      ...
      if (mounted) {
        SnackBarHelper.showSuccess(
          context,
          'Chuyển trạng thái: ${newStatus == 'inactive' ? 'Offline' : 'Online'}',
        );
      }
    } else {
      ...
      if (mounted) {
        SnackBarHelper.showSuccess(
          context,
          'Lỗi đổi trạng thái: ${response['message'] ?? 'Lỗi không xác định'}',
        );
      }
    }
  }
  ...
  Future<void> completeOrder(String idOrder) async {
    ...
    try {
      final data = {'orderId': idOrder, 'result': 'done'};
      ...
      if (response['success'] == true) {
        ...
        SnackBarHelper.showSuccess(
          context,
          "Bạn đã hoàn thành đơn thành công!",
        );
        ...
        SnackBarHelper.showError(
          context,
          response['message'] ?? "Có lỗi xảy ra khi hoàn thành đơn",
        );
      }
    } catch (e) {
  ...
  /// Tên dịch vụ
  String get _orderServiceName =>
      orderDetail?['nameService'] ?? 'Không xác định';

  /// Địa chỉ
  String get _orderAddress => orderDetail?['address'] ?? 'Không có địa chỉ';

  /// Giá tiền
  int get _expectedIncome =>
      (orderDetail?['pricing']['technicianReceiveAmount'] ?? 0) as int;
                        ...
                        Row(
                          ...
                          children: [
                            Container(
                              ...
                              child: ClipOval(
                                child:
                                    technicianData?['avatar']?['url'] != null
                                        ? Image.network(
                            ...
                            Stack(
                              ...
                              children: [
                                IconButton(
                                  onPressed: () {
                                    context.push(
                                      TechnicianRouterConfig.notifications,
                                    );
                                  },
                                  icon: const Icon(
                            ...
                            child: Row(
                              ...
                              children: [
                                _buildShortcutItem(
                                  ...
                                  onTap:
                                      () => context.push(
                                        TechnicianRouterConfig.historyOrder,
                                      ),
                                ),
                                _buildShortcutItem(
                                  ...
                                  onTap:
                                      () => context.push(
                                        TechnicianRouterConfig.statistical,
                                      ),
                                ),
                                _buildShortcutItem(
    ...
    return showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.my_location, color: ColorConfig.primary, size: 28),
              const SizedBox(width: 12),
              const Text(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow(
                      ...
                      'Khách hàng: ${customerId != null ? customerId["fullname"] : "Khách hàng"}',
                    ),
                    ...
                    _buildInfoRow(
                      Icons.phone,
                      'SĐT: ${customerId != null ? customerId["phone"] : orderDetail?["phoneCustomer"]}',
                    ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed:
                            () =>
                                _showCompleteOrderConfirmation(idOrderWorking),
