...
class RealtimeService {
  ...
  List<Function(dynamic data)> onNewTechnicianApplyOrderListeners = [];
  ...
  RealtimeService({
    ...
    List<Function(dynamic data)>? onNewTechnicianApplyOrderListeners,
  }) : onNewTechnicianApplyOrderListeners = onNewTechnicianApplyOrderListeners ?? [];
  ...
  Future<void> _handleEvent(dynamic event) async {
    try {
      ...
      if (data is Map<String, dynamic> && data['type'] == 'user_status_updated') {
      ...
      else if (data['type'] == 'new_automatching_order') {
        final order = data['data'];
        appLog("Data from service realtime: $order");
        ...
        appLog("Vo đây với data: $dataApply");
